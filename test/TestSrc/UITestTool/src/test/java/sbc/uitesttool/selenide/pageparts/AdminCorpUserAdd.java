...
public enum AdminCorpUserAdd{
   メールアドレス_項目($$(Selectors.byText("メールアドレス")).first()), //
   メールアドレス($("#mngAccountMail")), //
   パスワード_項目($$(Selectors.byText("パスワード")).first()), //
   パスワード_チェックボックス($("#createPwFlg")), //
   パスワード_テキストボックス($("#mngAccountPwd")), //
   この設定で登録する($("#doAddManageUser")), //
   キャンセル($("#doCancel")), //


	;

	private AdminCorpUserAdd(SelenideElement element) {
