...
public enum AdminDukeList{
   ユーザー番号_項目($$(Selectors.byText("ユーザー番号")).first()), //
   ニックネーム_項目($$(Selectors.byText("ニックネーム")).first()), //
   新規個別アカウントを追加_項目($$(Selectors.byText("新規個別アカウントを追加")).first()), //
   ログイン名_メールアドレス_項目($$(Selectors.byText("ログイン名(メールアドレス)")).first()), //
   ページ選択($("#paginationNo")), //
   ユーザー番号($("#goAdminmodusercorp")), //
   ニックネーム($("#displayName")), //
   ログイン名_メールアドレス($("#mailAddress")), //
   新規個別アカウントを追加($("#doAddBusinessUser")), //

   ユーザー番号0($$(Selectors.byText("()+.-;[]~=")).first()), //
   ユーザー番号1($$(Selectors.byText(")A10000001")).first()), //
   ユーザー番号2($$(Selectors.byText(")A19000001")).first()), //
   ニックネーム0($$(Selectors.byText("きごう")).first()), //
   ニックネーム1($$(Selectors.byText("自動個別アカウント")).first()), //
   ニックネーム2($$(Selectors.byText("自動個別アカウント_2")).first()), //
   ログイン名0($$(Selectors.byText("kigou@kigoumaru.com")).first()), //
   ログイン名1($$(Selectors.byText("autouser1@test.test")).first()), //
   ログイン名2($$(Selectors.byText("autouserx@test.test")).first()), //
   ニックネーム編集($$(Selectors.byText("自動個別アカウント編集")).first()), //
   ログイン名編集($$(Selectors.byText("cpiautouser1@test.test")).first()), //
	;

	private AdminDukeList(SelenideElement element) {
