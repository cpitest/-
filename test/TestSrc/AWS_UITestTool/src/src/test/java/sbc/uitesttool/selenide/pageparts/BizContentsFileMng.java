...
public enum BizContentsFileMng{
   ユーザー番号_項目($$(Selectors.byText("ユーザー番号")).first()), //
   コンテンツの種類_項目($$(Selectors.byText("コンテンツの種類")).first()), //
   ニックネーム_項目($$(Selectors.byText("ニックネーム")).first()), //
   ユーザー番号($("#userCode")), //
   コンテンツの種類($("#appKeyName")), //
   ニックネーム($("#displayName")), //
   ユーザー情報を変更する($("#doModUser")), //
   コンテンツ管理に戻る($("#doBack2ContentMng")), //
   すべて解除_上($("#allSelect")), //
   すべて選択_上($("#allCancel")), //
   選択したファイルを削除_上($("#doOnceDeleteFiles")), //
   登録名を変更する_上($("#doUpdateFileInfo")), //
   すべて解除_下($("#allSelect2")), //
   すべて選択_下($("#allCancel2")), //
   選択したファイルを削除_下($("#doOnceDeleteFiles2")), //
   登録名を変更する_下($("#doUpdateFileInfo2")), //
   有効期限_項目($$(Selectors.byText("有効期限")).first()), //
   登録名_項目($$(Selectors.byText("登録名")).first()), //
   ファイルリスト_プレビュー($("#thumbnailURL")), //
   ファイルリスト_チェックボックス($("#deleteFlg")), //
   ファイルリスト_有効期限($("#deleteDate")), //
   ファイルリスト_登録名($("#fileName")), //
   ファイルの有効期限($("#expireDate")), //
   ファイルアップロード_テキストボックス($("#uploadedFilesTxtBox")), //
   アップロード($("#doOnceUploadFiles")), //


	;

	private BizContentsFileMng(SelenideElement element) {
