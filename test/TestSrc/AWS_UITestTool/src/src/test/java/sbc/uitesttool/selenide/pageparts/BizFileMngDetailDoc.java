...
public enum BizFileMngDetailDoc{
   文書詳細画面($("#cboxContent")), //
   プレビュー($("#previewImg")), //
   ...
   ファイル名_項目($$(Selectors.byText("ファイル名")).first()), //
   カテゴリー_項目($$(Selectors.byText("カテゴリー")).first()), //
   有効期限_項目($$(Selectors.byText("有効期限")).first()), //
   ページ数_項目($$(Selectors.byText("ページ数")).first()), //
   ファイルサイズ_項目($$(Selectors.byText("ファイルサイズ")).first()), //
   用紙サイズ_項目($$(Selectors.byText("用紙サイズ")).first()), //
   印刷回数_項目($$(Selectors.byText("印刷回数")).first()), //
   メールアドレス($("#userMail")), //
   ユーザー番号($("#userCode")), //
   ニックネーム($("#displayName")), //
   登録先($("#registBoxName")), //
   ...
   閉じる($("#cboxClose")), //
	...
	private BizFileMngDetailDoc(SelenideElement element) {
