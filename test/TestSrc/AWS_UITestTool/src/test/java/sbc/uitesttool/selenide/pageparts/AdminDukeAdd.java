...
public enum AdminDukeAdd{
   ...
   ファイル保管日数_項目($$(Selectors.byText("ファイル保管日数")).first()), //
   店舗にあるマルチコピー機でのログイン方法_項目($$(Selectors.byText("店舗にあるマルチコピー機でのログイン方法")).first()), //
   後課金_コピー機画面カスタマイズ設定_項目($$(Selectors.byText("後課金/コピー機画面カスタマイズ設定")).first()), //
   パスワード有効期限設定_項目($$(Selectors.byText("パスワード有効期限設定")).first()), //
   ユーザー番号($("#userNo")), //
   ログイン名_メールアドレス($("#userMail")), //
   パスワード($("#userPwd")), //
   ニックネーム($("#userName")), //
   ファイル保管日数($("#fileStrorageLimit")), //
   店舗にあるマルチコピー機でのログイン方法_番号とログイン名($("#rbAuthType1")), //
   店舗にあるマルチコピー機でのログイン方法_ログイン名のみ($("#rbAuthType2")), //
   後課金_コピー機画面カスタマイズ設定($("#appKeyId")), //
   パスワード有効期限設定($("#setChangePwPeriodFlg")), //
   この設定を登録する($("#doAddBizUser")), //
   キャンセル($("#doCancel")), //


	;

	private AdminDukeAdd(SelenideElement element) {
