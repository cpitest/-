...
public enum BizContentsUserAdd{
   ユーザー番号_項目($$(Selectors.byText("ユーザー番号")).first()), //
   ユーザー番号の値を自動で発行する($("#userNoAuto")), //
   ユーザー番号の値を設定する($("#userNoManual")), //
   ユーザー番号の値を設定する_テキストボックス($("#userNoSuffix")), //
   ニックネーム_項目($$(Selectors.byText("ニックネーム")).first()), //
   ニックネーム($("#userName")), //
   コンテンツの種類_項目($$(Selectors.byText("コンテンツの種類")).first()), //
   コンテンツの種類($("#appKeyName")), //
   この内容で登録する($("#doOnceAddUser")), //
   キャンセル($("#doCancel")), //


	;

	private BizContentsUserAdd(SelenideElement element) {
