...
public enum AdminCorpModByLite{
   ...
   印刷料金を価格情報ファイルから設定する_項目($$(Selectors.byText("印刷料金を価格情報ファイルから設定する")).first()), //
   外部クラウドサービスとの連携_項目($$(Selectors.byText("外部クラウドサービスとの連携")).first()), //
   ユーザー番号のプレフィックス($("#userNoPrefix")), //
   企業名($("#corpName")), //
   法人アカウントの現在登録数($("#registedAccounts")), //
   ...
   印刷料金を価格情報ファイルから設定する_料金を確認($("#viewPrintPrice")), //
   印刷料金を価格情報ファイルから設定する_ファイルを選択($("#importPrintPriceFile")), //
   外部クラウドサービスとの連携($("#cooperationAppKey")), //
   メールアドレス_項目($$(Selectors.byText("メールアドレス")).first()), //
   メールアドレス($("#adminUserMail")), //
   管理者アカウントを追加する($("#doAddAdminAccount")), //
   この設定で変更する($("#doModUserCorp")), //
   この法人アカウントを削除する($("#doDelUserCorp")), //
   キャンセル($("#doCancel")), //


	;

	private AdminCorpModByLite(SelenideElement element) {
