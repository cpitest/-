...
public enum BizUserAccount{
   アカウントをCSVファイルで入力する_項目($$(Selectors.byText("アカウントをCSVファイルで入力する")).first()), //
   ファイル選択用テキストボックス($("#importFileTxtBox")), //
   CSVで入力($("#doImportCsv")), //
   アカウントをCSVファイルで出力する_項目($$(Selectors.byText("アカウントをCSVファイルで出力する")).first()), //
   CSVで出力($("#doExportCsv")), //


	;

	private BizUserAccount(SelenideElement element) {
