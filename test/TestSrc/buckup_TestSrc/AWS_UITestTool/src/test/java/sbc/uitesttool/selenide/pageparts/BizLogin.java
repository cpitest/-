...
public enum BizLogin{
   ログイン名($("#txtId")), //
   パスワード($("#txtPw")), //
   ログイン名を保存する($("#chkSaveId")), //
   ログインボタン($("#doLogin")), //
   ログイン失敗メッセージ($$(Selectors.byText("※ネットワークプリントサービスにログインできませんでした。ログイン名またはパスワードに間違いがないかご確認ください。")).first()), //

	;

	private BizLogin(SelenideElement element) {
