...
public enum UserCodesFileList{
   ステータスを更新する($("#doStatusUpdate")), //
   有効期限($$(Selectors.byText("有効期限")).first()), //
   登録名($$(Selectors.byText("登録名")).first()), //
   ページ数($$(Selectors.byText("ページ数")).first()), //
   ステータス($$(Selectors.byText("ステータス")).first()), //
   有効期限_項目($("#deleteDate")), //
   登録名_項目($("#fileName")), //
   ページ数_項目($("#totalPages")), //
   ステータス_項目($("#doFileStatus")), //


	;

	private UserCodesFileList(SelenideElement element) {
