...
public enum BizFileMng{
   登録日_項目($$(Selectors.byText("登録日")).first()), //
   登録日_前($("#startUploadDate")), //
   登録日_後($("#endUploadDate")), //
   ユーザー番号_ニックネーム_メールアドレス_項目($$(Selectors.byText("ユーザー番号／ニックネーム／メールアドレス")).first()), //
   ユーザー番号_ニックネーム_メールアドレス($("#targetUser")), //
   ファイル名_登録名_項目($$(Selectors.byText("ファイル名／登録名")).first()), //
   ファイル名_登録名($("#targetFile")), //
   登録先_項目($$(Selectors.byText("登録先")).first()), //
   登録先($("#registedBoxKey")), //
   検索する($("#doSearch")), //
   すべて選択_上($("#allCancel")), //
   すべて解除_上($("#allSelect")), //
   選択したファイルを削除_上($("#doDeleteFiles")), //
   ファイルリストをCSVで出力_上($("#doExportCsvFile")), //
   すべて選択_下($("#allCancel2")), //
   すべて解除_下($("#allSelect2")), //
   選択したファイルを削除_下($("#doDeleteFiles2")), //
   ファイルリストをCSVで出力_下($("#doExportCsvFile2")), //
   ファイルリスト_項目_登録日($$(Selectors.byText("登録日時")).first()), //
   ファイルリスト_項目_有効期限($$(Selectors.byText("有効期限")).first()), //
   ファイルリスト_項目_ユーザー番号($$(Selectors.byText("ユーザー番号")).first()), //
   ファイルリスト_項目_登録先($$(Selectors.byText("登録先")).first()), //
   ファイルリスト_項目_登録名($$(Selectors.byText("登録名")).first()), //
   ファイルリスト_項目_用紙サイズ($$(Selectors.byText("用紙")).first()), //
   ファイルリスト_項目_面数($$(Selectors.byText("面数")).first()), //
   ファイルリスト_項目_詳細($$(Selectors.byText("詳細")).first()), //
   ファイルリスト_チェックボックス($("#deleteFlg")), //
   ファイルリスト_登録日($("#createDate")), //
   ファイルリスト_有効期限($("#deleteDate")), //
   ファイルリスト_ユーザー番号($("#userCode")), //
   ファイルリスト_登録先($("#registBoxName")), //
   ファイルリスト_登録名($("#fileName")), //
   ファイルリスト_用紙サイズ($("#paperSize")), //
   ファイルリスト_面数($("#totalPages")), //
   ファイルリスト_詳細($("#FileDetail")), //
   ページ選択($("#paginationNo")), //


	;

	private BizFileMng(SelenideElement element) {
