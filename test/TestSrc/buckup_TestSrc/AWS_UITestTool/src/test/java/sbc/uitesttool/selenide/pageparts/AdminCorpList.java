...
public enum AdminCorpList{
   ユーザー番号のプレフィックス_項目($$(Selectors.byText("ユーザー番号のプレフィックス")).first()), //
   企業名_項目($$(Selectors.byText("企業名")).first()), //
   ユーザー番号のプレフィックス($$(Selectors.byText("))8")).first()), //
   企業名($$(By.linkText("自動テスト")).first()), //
   新規法人アカウントを追加ボタン($("#doAddUserCorp")), //

   forBiz用法人プレフィックス($$(Selectors.byText("]A1")).first()), //
   forBiz用法人企業名($$(By.linkText("自動テスト1")).first()), //
   forBiz用法人企業名_編集($$(By.linkText("]A1編集")).first()), //
   //forBiz用法人（プレフィックス_AT1／企業名_自動テスト1／メールアドレス_at1@test.test／パスワード_cpi12345）

   forBizLite用法人プレフィックス($$(Selectors.byText("]A2")).first()), //
   forBizLite用法人企業名($$(By.linkText("自動テスト2")).first()), //
   forBizLite用法人企業名_編集($$(By.linkText("]A2編集")).first()), //
   //forBizLite用法人（プレフィックス_AT2／企業名_自動テスト2／メールアドレス_at2@test.test／パスワード_cpi12345）

   コンテンツ利用用法人プレフィックス($$(Selectors.byText("]A3")).first()), //
   コンテンツ利用用法人企業名($$(By.linkText("自動テスト3")).first()), //
   コンテンツ利用用法人企業名_編集($$(By.linkText("]A3編集")).first()), //
   //コンテンツ利用用法人（プレフィックス_AT3／企業名_自動テスト3／メールアドレス_at3@test.test／パスワード_cpi12345）


   ;

	private AdminCorpList(SelenideElement element) {
